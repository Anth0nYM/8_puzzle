...
def bfs(initial_state, goal_state):
    queue = collections.deque([(initial_state, [])])  # Adicionando um caminho vazio inicial
    visited = set()
    ...
    while queue:
        ...
        if np.array_equal(node, goal_state):
            return path + [node]  # Retornando o caminho até o nó alvo
        for child in state.generate_children(node):
            child_tuple = state.to_tuple(child)
            if child_tuple not in visited:
                queue.append((child, path + [node]))  # Adicionando o caminho até o nó filho
...
def dfs(initial_state, goal_state):
    stack = collections.deque([(initial_state, [])])  # Adicionando um caminho vazio inicial
    visited = set()
    visited.add(state.to_tuple(initial_state))
    ...
    while stack:
        ...
        for child in state.generate_children(node):
            child_tuple = state.to_tuple(child)
            if child_tuple not in visited:
                stack.append((child, path + [node]))  # Adicionando o caminho até o nó filho
                visited.add(child_tuple)
